...
class BaselineRegistry(Registry):
    ...
    def register_trainer(cls, to_register=None, *, name: Optional[str] = None):
        ...
        """
        from habitat_baselines_.common.base_trainer import BaseTrainer
    ...
    @classmethod
    def register_encoder(cls, to_register=None, *, name: Optional[str] = None):
        r"""Register a supervised encoder (so auxiliary tasks can share encoders)

        Args:
            name: Key with which the env will be registered.
                If None will use the name of the class.
        """
        return cls._register_impl("supervised_encoder", to_register, name)

    @classmethod
    def get_encoder(cls, name):
        return cls._get_impl("supervised_encoder", name)
    ...
    def register_aux_task(cls, to_register=None, *, name: Optional[str] = None):
        ...
        Args:
            name: Key with which the env will be registered.
                If None will use the name of the class.
        """
        from habitat_baselines_.common.auxiliary_tasks.aux_utils import RolloutAuxTask
    ...
    @classmethod
    def get_aux_tasks(cls, aux_cfg):
        name_list = aux_cfg.tasks
        task_classes = []
        encoder_classes = {}
        for name in name_list:
            task_classes.append(cls.get_aux_task(name))
            for req_sensor in task_classes[-1].get_required_sensors(aux_cfg[name]):
                if req_sensor not in encoder_classes:
                    encoder_classes[req_sensor] = cls.get_encoder(req_sensor)
        return task_classes, encoder_classes
    ...
    def register_policy(cls, to_register=None, *, name: Optional[str] = None):
        ...
        """
        from habitat_baselines_.rl.ppo.policy import Policy
    ...
    def register_obs_transformer(
        ...
        """
        from habitat_baselines_.common.obs_transformers import (
